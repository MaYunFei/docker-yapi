...
function init-network(){
    ...
    sudo docker network create --subnet=172.19.0.0/16 tools-net
}
...
function start-yapi(){
    ...
    sudo docker run -d -p 3001:3001 --name yapi --net tools-net --ip 172.19.0.3 yapi
    echo "end yapi server"
}

function init-yapi(){
    echo "init yapi db and start yapi server"
    sudo docker run -d -p 3001:3001 --name yapi --net tools-net --ip 172.19.0.3 yapi --initdb
    echo "init yapi done"
}
